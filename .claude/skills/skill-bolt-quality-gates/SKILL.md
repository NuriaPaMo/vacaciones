...
If your service name doesn't follow `kebab-case → PascalCase` (e.g. `autenticacion-autorizacion →
Auth`), add the mapping to `$ServiceNameMap` in the Quality-Gates script, or pass
`-BoltScope <ServiceName>` / `--bolt-scope <name>` explicitly.
