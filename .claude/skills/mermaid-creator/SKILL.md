...
Configure diagrams using YAML frontmatter **at the top of the `.mmd` file**, before the diagram type.

### Default Inline Style

**Always apply this frontmatter block** unless the user explicitly requests a different theme.
It uses `base` theme with neutral tones that render well in both light and dark environments,
and in GitHub Markdown, Confluence, and VS Code previews:
...
| Theme | Best for |
|---|---|
| `base` | Customizable — use with `themeVariables` (default) |
| `default` | Light, clean, general documentation |
| `forest` | Green palette, nature/sustainability domains |
| `dark` | Dark backgrounds, terminal-style docs |
| `neutral` | Monochrome, formal/enterprise reports |

### themeVariables Reference

```yaml
themeVariables:
  primaryColor: "#4f46e5"       # Main node fill
  primaryTextColor: "#ffffff"   # Text inside primary nodes
  primaryBorderColor: "#3730a3" # Border of primary nodes
  lineColor: "#6b7280"          # Arrow/connection color
  secondaryColor: "#f3f4f6"     # Secondary node fill
  tertiaryColor: "#e0e7ff"      # Tertiary/accent fill
...
```yaml
config:
  look: classic      # Traditional Mermaid (default)
...
```yaml
config:
  layout: dagre   # Balanced, default — works for most diagrams
  # layout: elk   # Advanced layout for dense graphs (requires integration)
```
